<script type="text/javascript">
document.write("test");
jQuery.getFeed({
	   //url: 'http://open.nysenate.gov/legislation/api/rss/search/otype%253Abill%2BAND%2B%2528year%253A2011%2BOR%2Bwhen%253A%255B1293840000281%2BTO%2B1356998399281%255D%2529%2BAND%2Bactive%253Atrue/'
	   url: "http://localhost:8080/mockups/templates/engadget_rss.xml",
	   success: function(feed)
	   {
		   document.write("success");
	     alert(feed.title);
	   }
	 });
document.write("test");
</script>
<br>
<div class="tab4">
<h3>The next scheduled senate is:</h3>
<h1>Jan 1, 2013</h1>
</div>
<br>
<br>
<br>

<div class="tabbable"> 
  <ul class="nav nav-tabs">
    <li class="active"><a href="#tab1" data-toggle="tab">Recently Passed</a></li>
    <li><a href="#tab2" data-toggle="tab">Most viewed</a></li>
    <li><a href="#tab3" data-toggle="tab">Signed into law</a></li>
  </ul>
  <div class="tab-content">
    <div class="tab-pane active" id="tab1">
     <p><b>BILL S21-2011: Relates to payment of independent contractors</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">LABOR</a>
<br>
Relates to payment of independent contractors and authorizes the department of labor to investigate complaints, make claims for compensation, assess liquid damages, civil penalties and criminal penalties, and authorizes the award of attorney fees and liquidated damages.
</p>
<p><b>BILL S22-2011: Provides that the income level for the requirement to file a personal income tax return shall be set by regulation</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">WAYS AND MEANS</a>
<br>
Provides that the income level for the requirement to file a personal income tax return shall be the dollar amount of the New York standard deduction.
</p>
<p><b>BILL S23-2011: Relates to the sale of tonic water, bitters and maraschino cherries at liquor stores</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">COMMERCE, ECONOMIC DEVELOPMENT AND SMALL BUSINESS</a>
<br>
Relates to the sale of tonic water, bitters and maraschino cherries at liquor stores.
</p>
</div>
    <div class="tab-pane" id="tab2">
     <p><b>BILL S24-2011: Requires certification notice to tenant when rent is paid by human resources administration</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">CITIES</a>
<br>
Provides that whenever rent is paid by NYC human resources administration on behalf of a tenant on public assistance, the department of housing preservation shall send a certification of payment to such tenant.
</p>
<p><b>BILL S25-2011: Relates to making the provisions governing liquor licenses consistent with respect to public interest factors</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">COMMERCE, ECONOMIC DEVELOPMENT AND SMALL BUSINESS</a>
<br>
Relates to making the provisions governing liquor licenses consistent with respect to public interest factors.
</p>
<p><b>BILL S26-2011: Relates to the procedure for filling a vacancy in the office of state senator or in the office of member of assembly</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">ELECTIOINS</a>
<br>
Relates to the procedure for filling a vacancy in the office of state senator or in the office of member of assembly; allows for independent nominating petitions to be presented and provides for a special election to be held forty-five days from the date of the governor making a proclamation for a special election.
</p>
</div>
    <div class="tab-pane" id="tab3">
     <p><b>BILL S27-2011: Establishes the Asian Lunar New Year day as a school holiday for all city school districts of one million or more with an Asian population of 7.5% or more</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">RULES</a>
<br>
Establishes the Asian Lunar New Year day as a school holiday for all city school districts of cities of one million inhabitants or more, with an Asian population of seven and one-half percent or more; uses the year 2000 federal decennial census as the basis for calculation of the Asian population.
</p>
<p><b>BILL S28-2011: Designates venue of certain special proceedings against certain state bodies or officers</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">JUDICIARY</a>
<br>
Permits special proceedings against certain designated state officials to be commenced in a supreme court in the judicial district of the petitioner's residence or principal place of business; provides that a proceeding against the public service commission shall only be commenced in the supreme court, Albany county.
</p>
<p><b>BILL S29-2011: Provides that in cities with a population over one million, required distances of bars from schools, places of worship and existing licensed premises be measured from the closest boundary lines</b>
<br>
Sponsor: <a href="#">SQUADRON</a> Committee: <a href="#">COMMERCE, ECONOMIC DEVELOPMENT AND SMALL BUSINESS</a>
<br>
Relates to the siting of certain premises, located in cities with a population in excess of one million, licensed to sell liquor for consumption on the premises; provides that required distances from schools and places of worship and from existing licensed premises shall be measured from the closest boundary lines instead of from the center of the entrances of such premises.
 </p>
     </div>
  </div>
</div>